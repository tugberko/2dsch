...
def nthTerm(n, some_dt):
    matrix = 1j * HMatrix() * some_dt * 0.5
    matrixPower = np.linalg.matrix_power(matrix, n)
    coef = 1/(math.factorial(n))
    return coef * matrixPower
...
def LHS(some_dt):
    return la.expm(1j * HMatrix() * some_dt * 0.5)

def RHS(some_dt):
    return la.expm(-1j * HMatrix() * some_dt * 0.5)


def InitializeU(some_dt):
    ustart = time.time()
    LHSmatrix = LHS(some_dt)
    RHSmatrix = RHS(some_dt)
...
def Run(some_dt):
    filename = 'dt' + str(some_dt) + '(' +str(nx) + 'x' + str(ny) + ').dat'
    ...
    timesteps = math.ceil(terminateAt / some_dt)



    U = InitializeU(some_dt)
    ...
    while (currentTime < terminateAt):
        ...
        #Evolve
        psi_num = np.matmul(U, np.ndarray.flatten(psi_num)).reshape(ny,nx)
...
for temporal_res in [ 1e-1 , 1e-2 , 1e-3 ]:
    Run(temporal_res)
